...
def irl(feature_matrix, n_actions, discount, transition_probability,
        trajectories, epochs, learning_rate, save_file_name, load_file_name, load_start_epoch):
    """
    ...
    if load_file_name is not None:
        alpha=pkl.load(open(load_file_name, 'rb'))
    ...
    for i in range(start_epoch, epochs):
        ...
        # pkl.dump(alpha, open("alpha_%d.pkl"%(i), 'wb'))
        pkl.dump(alpha, open(save_file_name % (i), 'wb'))

        # r = feature_matrix.dot(alpha).reshape((n_states,))
        # draw_airplane_reward('reward_epoch%d'%(i), r, True)
